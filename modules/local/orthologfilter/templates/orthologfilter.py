...
with open("$assembly_blast", "r") as f:
    scaffolds = {}
    for line in f.readlines():
        (
            qseqid,
...
for probe, probe_values in probes.items():
    ...
    scaffolds_eval = {
        key: value for key, value in scaffolds.items() if value["locus"] == probe
    }
...
if "$prefix":
    with open("$prefix" + ".ortho_probe.bed", "w") as ortho_probe:
        with open("$prefix" + ".ortho_full.bed", "w") as ortho_full:
            for x in keep:
